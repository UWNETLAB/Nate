...
def mp(items, function, cpu, *args):
    ...
    temp = Parallel(n_jobs=cpu, verbose=10, max_nbytes=None)(delayed(function)(
...
def dissim(auth_list, auth_alt_dict, auth_alt_dict_2, auth_vectors):
    ...
    for author in auth_list:
        row_dict = {}
        if author in auth_alt_dict and len(
                auth_alt_dict[author]
        ) > 0:  # check that author has alters ('NA' if not)
            alter_list = auth_alt_dict[author]
            alter_avg_diff = single_avg_dissim(
                author, alter_list,
                auth_vectors)  # send to create_alter_avg_diff function
        else:
            alter_avg_diff = 'NA'
        if author in auth_alt_dict_2 and len(
                auth_alt_dict_2[author]
        ) > 0:  # check that author has alters_2 ('NA' if not)
            alter_2_list = auth_alt_dict_2[author]
            alter_2_avg_diff = single_avg_dissim(
                author, alter_2_list,
                auth_vectors)  # send to create_alter_avg_diff function
        else:
            alter_2_avg_diff = 'NA'
        ...
        row_dict = {
            'author': author,
            'dissim_alters': alter_avg_diff,
            'dissim_alters_2': alter_2_avg_diff
        }
...
def single_avg_dissim(ego, alter_list, vectors):
    ...
    if len(alter_list) > 1:
        ...
        ego_dissim = 1 - linear_kernel(
            ego_vector, v_array
        )  # pairwise comparison of ego vector to all alter vectors in the array
                ...
                if len(auth_alt_dict[alter]) > 1:
                    ...
                    if len(alter_2_list_trim) > 0:
                        alter_dissim = single_avg_dissim(
                            alter, alter_2_list_trim, auth_vectors)
                        rb_dissims.append(alter_dissim)
                    if len(ring_list) > 0:
                        alter_dissim = single_avg_dissim(
                            alter, ring_list, auth_vectors)
                        ring_dissims.append(alter_dissim)
                    if len(bridge_list_trim) > 0:
                        alter_dissim = single_avg_dissim(
                            alter, bridge_list_trim, auth_vectors)
            ...
            for alter in alter_list:
                if len(auth_alt_dict[alter]) > 1:
                    ...
                    if len(alter_2_list) > 0:
                        alter_dissim = single_avg_dissim(
                            alter, alter_2_list, auth_vectors)
                        dissims.append(alter_dissim)
        if len(dissims) > 0:
            avg_dissims.append(np.round(np.average(dissims), 3))
        else:
